...
//showBMP("/miniwoof.bmp", 5, 5);delay(1000);
//showBMP("/247418~1.bmp", 5, 5);delay(1000);
//showBMP("/147187~1.bmp", 5, 5);delay(1000);
recvWithEndMarker();
showNewData();
...
void showNewData() {
 if (newData == true) {
  ...
  String te=readname(receivedChars);
  Serial.println(te);
 ...
 showBMP(cstr, 5, 5);
 tft.setTextSize(4);
 tft.setCursor(11,260);
 tft.println("APPROVED");
 delay(3500);
 while(digitalRead(19)==0){delay(500);}
...
String ans="";
  ...
  if (myFile) {
    ...
    while (myFile.available()) {uname="";
      uname=String(char(myFile.read()));
      ans=ans+uname;
    }
    // close the file:
    return ans;
